...
public class CandyCrushGame {
    ...
    private boolean checkAndHandleMatches() {
        ...
        for (int row = 0; row < ROWS; row++) {
            for (int col = 0; col < COLS - 2; col++) {
                int type = Integer.parseInt(buttons[row][col].getActionCommand());
                if (type == Integer.parseInt(buttons[row][col + 1].getActionCommand()) &&
                        type == Integer.parseInt(buttons[row][col + 2].getActionCommand())) {
                    hasMatches = true;
                    clearCandy(row, col);
                    clearCandy(row, col + 1);
                    clearCandy(row, col + 2);
                    score += 30;
                }
            }
        }
        ...
        for (int col = 0; col < COLS; col++) {
            for (int row = 0; row < ROWS - 2; row++) {
                int type = Integer.parseInt(buttons[row][col].getActionCommand());
                if (type == Integer.parseInt(buttons[row + 1][col].getActionCommand()) &&
                        type == Integer.parseInt(buttons[row + 2][col].getActionCommand())) {
                    hasMatches = true;
                    clearCandy(row, col);
                    clearCandy(row + 1, col);
                    clearCandy(row + 2, col);
                    score += 30;
                }
            }
        }
    ...
    private void clearCandy(int row, int col) {
        buttons[row][col].setActionCommand("");
